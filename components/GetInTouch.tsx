...
const schema = z.object({
    ...
    phoneNumber: z.string().min(3, "Invalid phone number"),
...
const GetInTouch = () => {
    ...
    const onSubmit = async (data: FormData) => {
        try {
            ...
            setIsLoading(false);
            toast.success('Thank you for contacting suwappify support, we will revert shortly')
            reset()
        } catch (error) {
            console.error(error);
            setIsLoading(false);
            toast.error('Error contacting support, try again')
        };
    }
                ...
                <div className="md:w-1/2">
                    ...
                    <form action="" className="w-full flex flex-col gap-4" onSubmit={handleSubmit(onSubmit)}>
                        <div className="w-full">
                            ...
                            <input type="text" className="w-full p-2.5 rounded-lg border border-[#5F5F53]" placeholder="Enter your name" {...register("name")} />
                            {errors.name && <p className='text-sm text-red-500'>{errors.name.message as React.ReactNode}</p>}
                        </div>

                        <div className="w-full">
                            <label className="md:text-lg text-[#181D17]">Email</label>
                            <input type="text" className="w-full p-2.5 rounded-lg border border-[#5F5F53]" placeholder="Enter your email address" {...register("email")} />
                            {errors.email && <p className='text-sm text-red-500'>{errors.email.message as React.ReactNode}</p>}

                        </div>

                        <div className="w-full">
                            <label className="md:text-lg text-[#181D17]">Phone Number</label>
                            <input type="text" className="w-full p-2.5 rounded-lg border border-[#5F5F53]" placeholder="Enter your Phone number" {...register("phoneNumber")} />
                            {errors.phoneNumber && <p className='text-sm text-red-500'>{errors.phoneNumber.message as React.ReactNode}</p>}

                        </div>

                        <div className="w-full flex flex-col">
                            <label className="md:text-lg text-[#181D17]">Message</label>
                            <textarea placeholder="Enter your message" className="h-[139px] border border-[#5F5F53] rounded-lg p-3 py-3" {...register("message")}></textarea>
                            {errors.message && <p className='text-sm text-red-500'>{errors.message.message as React.ReactNode}</p>}
                        </div>

                        <button className="bg-[#50644C] py-3 px-6 rounded-xl text-white flex justify-center items-center gap-3 md:text-lg">{!isLoading ? (
                            <>
                            Send message
                            <ArrowRight />
                            </>
